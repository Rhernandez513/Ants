...
//  GameBlock *tempBlock =
//      field.GetBlock(ant1->GetLocation());  // setting tempBlock equal to the
//                                            // ants location on the block
...
//}
// else {
// if (field[position1][position2].a->color == field[&position1 + i][&position2
// + j].a->color)
...
choose another one
                                ...
                                pickant(x, y, Color::red); // I guess he was
trying to use the "pickant" function to take a turn
                                res = CloseAnt(x, y); // don't know what's going
on here
                                if (res == -2)  // ant we picked is blocked,
choose another one
        ...
        // returns: -1: both queens killed, draw. 0: both ants killed, game not
over
        // yet. 1:first ant killed second. 2: second killed first. 3: queen 2
killed,
        // team 1 won. 4: queen 1 killed, team 2 won
int AntMover::combat(Position position, GameField &field) {
        // Majed set the positions of the two queens on either side of the
board, they were originally called "a1" "a2"
        int r;
        srand(time(NULL));
        r = rand() % 3;
